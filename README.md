...
| --------- | ----------- | ------- |
| **KEYCLOAK_ROUTE_HOSTNAME** | The external hostname to access the iPaaS Keycloak | syndesis-keycloak.127.0.0.1.xip.io |
...
| **KEYCLOAK_IPAAS_REALM_NAME** | iPaaS Keycloak realm name | syndesis |
...
Once everything is running, you should be able to access iPaaS at https://syndesis.127.0.0.1.nip.io and log in with the OpenShift user `developer` using any password.
...
Replace `EXTERNAL_HOSTNAME` appropriately with your public iPaaS address (something like `syndesis.127.0.0.1.nip.io` works great if you are using `oc cluster up` locally).
